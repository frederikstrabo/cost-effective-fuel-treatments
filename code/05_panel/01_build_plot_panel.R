...
################# Purpose: This code develops the function "create_SpatialDiD_Grids" which for each fire 
#################           in our sample creates radial plots and their associated wildfire outcomes and 
#################           relevant covariates to be used in our spatial DiD analysis. The function allows 
#################           flexibility to change the number of radial directions = L, the distance of each plot = K.
...
## LAT Drops - Note if the user doesn't have access to the LAT drops comment this out
...
# L = number of direction, K = length of distance bin in km, Tau = percent of a plot that needs to be treated to be considered "Treated"

create_SpatialDiD_Grids <- function(L, K, Tau, CENT){
  
  SpatialOLS_Grids <- data.frame(matrix(ncol = 58, nrow = 0), geometry = st_sfc())
  ...
  for (i in 1:nrow(fires_int_df)){
    ...
    Grids_Treated <- Grids_Treated_Int %>%
      ...
      mutate(Pct_Treated = Acres_Treated/Grid_Acres,
             Treated = ifelse(Pct_Treated > Tau, 1, 0)) %>%
    ...
    Grids <- mutate(Grids, distance_bin_end = distance_bin_end/(K*1000))
    ...
    # Grids_NSO_Int <- st_intersection(Grids, NSO)
    # 
    # if (nrow(Grids_NSO_Int) > 0){
    #   
    #   Grids_NSO_Int$NSO_Acres <- as.numeric(st_area(Grids_NSO_Int)/4046.86)
    #   
    #   Grids_NSO_Int <- Grids_NSO_Int %>%
    #     as.data.frame() %>%
    #     group_by(Grid_ID) %>%
    #     summarise(NSO_Acres = sum(NSO_Acres)) 
    #   
    #   Grids <- merge(Grids, Grids_NSO_Int, by = "Grid_ID", all.x = T)
    #   Grids[is.na(Grids$NSO_Acres), "NSO_Acres"] <- 0
    #   
    # }
    # 
    # if (nrow(Grids_NSO_Int) == 0){
    ...
    Grids$BURN_SEV_HIGH_PCT <- exact_extract(
      mtbsBS_year,
      Grids_reproj,
      fun = burn_sev_4_pct,
      summarize_df = TRUE,
    ...
    Grids[is.na(Grids$BURN_SEV_HIGH_PCT), "BURN_SEV_HIGH_PCT"] <- 0
    ...
    if (nrow(QAQC_fire) == 0){
      ...
      Grids <- mutate(Grids, SUP_LINES_PRESENT = 0)
      Grids$SUP_LINE_DIST <- NA
      Grids$SUP_LINE <- NA
      Grids$LOG_SUP_LINE_DIST <- NA
      Grids$SUP_LINE_LEN <- NA
      Grids$SUP_LINE_INT <- NA
    }
    
    if (nrow(QAQC_fire) != 0){
      
      Grids <- mutate(Grids, SUP_LINES_PRESENT = 1)
      Grids$SUP_LINE_DIST <- calculate_distance(Grids, QAQC_fire)
      Grids <- mutate(Grids, SUP_LINE = ifelse(SUP_LINE_DIST == 0, 1, 0),
      ...
      lines_in_grids <- st_intersection(QAQC_fire, Grids)
      ...
      Grids <- merge(Grids, grid_line_lengths, by = "Grid_ID", all.x = T)
      
      Grids[is.na(Grids$SUP_LINE_LEN), "SUP_LINE_LEN"] <- 0
      
      Grids <- mutate(Grids, SUP_LINE_INT = SUP_LINE_LEN/Grid_Acres)
  ...
  SpatialOLS_Grids <- merge(SpatialOLS_Grids, smoke_costs, by = "FIRE_ID", all.x = T)
  
  SpatialOLS_Grids <- mutate(SpatialOLS_Grids, Deaths_PER_ACRE = Deaths/FIRE_SIZE,
  ...
  #### iii) Fire Regime Characteristics (MFRI),  Fuel Model Type & OGSI Class
  ...
  # # OGSI
  # 
  # OLS_Grids_reproj <- st_transform(OLS_Grids, crs = st_crs(OGSI))
  # 
  # # Define function to compute percent of values >= 1 i.e. OGSI 80
  # extract_pct_gte1 <- function(df) {
  #   valid <- !is.na(df$value)
  #   sum(df$coverage_fraction[valid & df$value >= 1]) / sum(df$coverage_fraction[valid])
  # }
  # 
  ...
  #     OGSI,
  #     filter(OLS_Grids_reproj, group_id == i),
  #     fun = extract_pct_gte1,
  #     summarize_df = TRUE,
  #     progress = FALSE
  #   ),
  #   mc.cores = (num_cores + 1)
  # )
  # 
  # # Combine and assign results
  # OLS_Grids$PCT_OGSI_CLS_80 <- unlist(temp)
  # 
  # rm(OGSI)
  # 
  # OLS_Grids$PCT_OGSI_CLS_80 <- as.numeric(OLS_Grids$PCT_OGSI_CLS_80)
  # 
  # OLS_Grids[is.na(OLS_Grids$PCT_OGSI_CLS_80), "PCT_OGSI_CLS_80"] <- 0
  # 
  # OLS_Grids <- mutate(OLS_Grids, ACRES_OGSI_80 = Grid_Acres*PCT_OGSI_CLS_80)
...
# OLS_Grids_df_L48_K05 <- create_SpatialDiD_Grids(L = 48, K = 0.5, Tau = 0.5)
OLS_Grids_df_L24_K05 <- create_SpatialDiD_Grids(L = 24, K = 0.5, Tau = 0.5, CENT = TRUE)
# OLS_Grids_df_L12_K05 <- create_SpatialDiD_Grids(L = 12, K = 0.5, Tau = 0.5)
# OLS_Grids_df_L48_K025 <- create_SpatialDiD_Grids(L = 48, K = 0.25, Tau = 0.5)

# OLS_Grids_df_L48_K05 <- create_SpatialDiD_Grids(L = 48, K = 0.50, Tau = 0.5)
# OLS_Grids_df_L36_K05 <- create_SpatialDiD_Grids(L = 36, K = 0.50, Tau = 0.5)
# OLS_Grids_df_L24_K05 <- create_SpatialDiD_Grids(L = 24, K = 0.50, Tau = 0.5)
# OLS_Grids_df_L16_K05 <- create_SpatialDiD_Grids(L = 16, K = 0.50, Tau = 0.5)
# OLS_Grids_df_L8_K05 <- create_SpatialDiD_Grids(L = 8, K = 0.50, Tau = 0.5)

# OLS_Grids_df_L48_K05_zero <- create_SpatialDiD_Grids(L = 48, K = 0.50, Tau = 0)
# OLS_Grids_df_L24_K05_zero <- create_SpatialDiD_Grids(L = 24, K = 0.50, Tau = 0)
# OLS_Grids_df_L16_K05_zero <- create_SpatialDiD_Grids(L = 16, K = 0.50, Tau = 0)
# OLS_Grids_df_L8_K05_zero <- create_SpatialDiD_Grids(L = 8, K = 0.50, Tau = 0)

# OLS_Grids_df_L48_K05_25p <- create_SpatialDiD_Grids(L = 48, K = 0.50, Tau = 0.25)
# OLS_Grids_df_L24_K05_25p <- create_SpatialDiD_Grids(L = 24, K = 0.50, Tau = 0.25)
# OLS_Grids_df_L16_K05_25p <- create_SpatialDiD_Grids(L = 16, K = 0.50, Tau = 0.25)
# OLS_Grids_df_L8_K05_25p <- create_SpatialDiD_Grids(L = 8, K = 0.50, Tau = 0.25)


# OLS_Grids_df_L36_K05_60P <- create_SpatialDiD_Grids(L = 36, K = 0.50, Tau = 0.6)
# OLS_Grids_df_L18_K05_40P <- create_SpatialDiD_Grids(L = 18, K = 0.50, Tau = 0.4)
# OLS_Grids_df_L12_K05_30P <- create_SpatialDiD_Grids(L = 12, K = 0.50, Tau = 0.3)
# OLS_Grids_df_L8_K05_20P <- create_SpatialDiD_Grids(L = 8, K = 0.50, Tau = 0.2)


# OLS_Grids_df_L24_K05_100p <- create_SpatialDiD_Grids(L = 24, K = 0.50, Tau = 0.99, T)
# OLS_Grids_df_L24_K05_75p <- create_SpatialDiD_Grids(L = 24, K = 0.50, Tau = 0.75, T)
# OLS_Grids_df_L24_K05_25p <- create_SpatialDiD_Grids(L = 24, K = 0.50, Tau = 0.25, T)
# OLS_Grids_df_L24_K05_zero <- create_SpatialDiD_Grids(L = 24, K = 0.50, Tau = 0, T)

# write_csv(OLS_Grids_df_L24_K05_100p, here("data", "intermediate", "SpatialDiD_Grids_L24_K05_100p.csv"))
# write_csv(OLS_Grids_df_L24_K05_75p, here("data", "intermediate", "SpatialDiD_Grids_L24_K05_75p.csv"))
# write_csv(OLS_Grids_df_L24_K05_25p, here("data", "intermediate", "SpatialDiD_Grids_L24_K05_25p.csv"))
# write_csv(OLS_Grids_df_L24_K05_zero, here("data", "intermediate", "SpatialDiD_Grids_L24_K05_zero.csv"))


# OLS_Grids_df_L24_K05_V2 <- create_SpatialDiD_Grids(L = 24, K = 0.50, Tau = 0.5, F)
# 
# write_csv(OLS_Grids_df_L24_K05_V2, here("data", "intermediate", "SpatialDiD_Grids_L24_K05_V2.csv"))


write_csv(OLS_Grids_df_L24_K05, here("data", "temp", "SpatialDiD_Grids_L24_K05.csv")) # Put the main sample in the "temp" directory to make sure to do smoke imputation in "DescriptiveStats.R" before analysis
# write_csv(OLS_Grids_df_L12_K05, here("data", "intermediate", "SpatialDiD_Grids_L12_K05.csv"))
# write_csv(OLS_Grids_df_L48_K025, here("data", "intermediate", "SpatialDiD_Grids_L48_K025.csv"))
# write_csv(OLS_Grids_df_L48_K10, here("data", "intermediate", "SpatialDiD_Grids_L48_K10.csv"))

# write_csv(OLS_Grids_df_L48_K05, here("data", "intermediate", "SpatialDiD_Grids_L48_K05.csv"))
# write_csv(OLS_Grids_df_L36_K05, here("data", "intermediate", "SpatialDiD_Grids_L36_K05.csv"))
# write_csv(OLS_Grids_df_L24_K05, here("data", "intermediate", "SpatialDiD_Grids_L24_K05.csv"))
# write_csv(OLS_Grids_df_L16_K05, here("data", "intermediate", "SpatialDiD_Grids_L16_K05.csv"))
# write_csv(OLS_Grids_df_L8_K05, here("data", "intermediate", "SpatialDiD_Grids_L8_K05.csv"))

# write_csv(OLS_Grids_df_L48_K05_zero, here("data", "intermediate", "SpatialDiD_Grids_L48_K05_zero.csv"))
# write_csv(OLS_Grids_df_L24_K05_zero, here("data", "intermediate", "SpatialDiD_Grids_L24_K05_zero.csv"))
# write_csv(OLS_Grids_df_L16_K05_zero, here("data", "intermediate", "SpatialDiD_Grids_L16_K05_zero.csv"))
# write_csv(OLS_Grids_df_L8_K05_zero, here("data", "intermediate", "SpatialDiD_Grids_L8_K05_zero.csv"))

# write_csv(OLS_Grids_df_L48_K05_25p, here("data", "intermediate", "SpatialDiD_Grids_L48_K05_25p.csv"))
# write_csv(OLS_Grids_df_L24_K05_25p, here("data", "intermediate", "SpatialDiD_Grids_L24_K05_25p.csv"))
# write_csv(OLS_Grids_df_L16_K05_25p, here("data", "intermediate", "SpatialDiD_Grids_L16_K05_25p.csv"))
# write_csv(OLS_Grids_df_L8_K05_25p, here("data", "intermediate", "SpatialDiD_Grids_L8_K05_25p.csv"))


# write_csv(OLS_Grids_df_L36_K05_60P, here("data", "intermediate", "SpatialDiD_Grids_L36_K05_60p.csv"))
# write_csv(OLS_Grids_df_L18_K05_40P, here("data", "intermediate", "SpatialDiD_Grids_L18_K05_40p.csv"))
# write_csv(OLS_Grids_df_L12_K05_30P, here("data", "intermediate", "SpatialDiD_Grids_L12_K05_30p.csv"))
# write_csv(OLS_Grids_df_L8_K05_20P, here("data", "intermediate", "SpatialDiD_Grids_L8_K05_20p.csv"))