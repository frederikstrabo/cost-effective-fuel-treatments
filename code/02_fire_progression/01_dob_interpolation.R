...
data_directory <- file.path(choose.dir())
data_directory <- gsub('\\\\', '/', data_directory) #for windows
...
sub_folders <- c(
  firms = 'firms_fire_detections', #FIRMS data from https://firms.modaps.eosdis.nasa.gov/download
  perim = 'mtbs_perims', #MTBS fire perimeters of an area of interst, or all from https://mtbs.gov/direct-download
  fire_list = 'selected_fires', #List of fires of interest with a mtbs ID column
  fire_id_column = 'MTBS_ID', #name of the column that corresponds to the name/ID of the fire in the .csv of fires
  output = '/data/interim_data' #output location
  )
...
nifc_perims = F
output_directory = paste0(data_directory, sub_folders[['output']])
...
hotspotsModis <- st_read(paste0(sub_folders[['firms']], '/modis'))
hotspotsModis <- subset(hotspotsModis, select = -c(TYPE))

## Get VIIRS fire detections
hotspotsViirsSpp <- st_read(paste0(sub_folders[['firms']], '/viirs'), 'fire_archive_SV-C2_431932')
hotspotsViirsSpp <- subset(hotspotsViirsSpp, select = -c(TYPE))
hotspotsViirsNoaa20 <- st_read(paste0(sub_folders[['firms']], '/viirs'), 'fire_nrt_J1V-C2_458502')
hotspotsViirsNoaa21 <- st_read(paste0(sub_folders[['firms']], '/viirs'), 'fire_nrt_J2V-C2_458503')
...
fires <- read.csv(list.files(paste0(sub_folders[['fire_list']]), full.names = T)[1])
# test on one fire fires <- subset(fires, Name == "ANTELOPE")
fires$Ignition <- as.Date(fires$Ignition) # Add a year column
...
fire_hotspots <- function(year) {
  ...
  for (xx in 1:dim(fire.list)[1]) {
    ...
    if (nrow(fire.hotspots) > 0 ) {
      ...
      max.date <- fire.list$yday[xx]+60 #two months post-ignition
...
# List all dob fires 
setwd(paste0(data_directory, '/data/interim_data/date_of_burn/dob_tif'))
if (nifc_perims == T) {
...
# SCRAP CODE
# This code double checks days are correctly saved

# fires  <- read.csv(paste0(output_directory, '/fire_lists/selectedfires_list_v3.csv'))
# fires$Ignition <- as.Date(fires$Ignition) # Add a year column
# fires$year <- lubridate::year(fires$Ignition)
# years <- unique(fires$year) # Run all years 2001 - 2021

# 
# for (y in 1:length(dob_stack)) {
#   dob_df <- as.data.frame(dob_stack[[y]], xy = T)
# 
#   days <- na.omit(dob_df$DOB)%% 100
#   range_days <- range(days)
#   print(range_days)
# 
# }
# 
#   
#   dob_df <- as.data.frame(large_rast, xy = T)
#   
#   days <- na.omit(dob_df$DOB)%% 100
#   range_days <- range(days)
#   print(range_days)
...
# just another way to check values in a big nasty raster
# big_nasty <- rast(paste0(output_directory,'/date_of_burn/dob_by_years/dob_2020.tif'))
# big_nasty_days <- as.data.frame(big_nasty) %>%
#   na.omit(dob_df$DOB)%% 100 %>%
#   unique()
